"""
Unified AOC-2 Processor - Fargate Version
Processes AOC-2 financial statement and fills Excel template in a single execution
...
from services.bedrock_extraction_service_unified import extract_aoc4_fields_with_bedrock  # Can be reused for AOC-2
...
def process_aoc2(bucket, document_key, template_key):
    """
    Main processing function for AOC-2
    ...
    log_info("=" * 80)
    log_info("UNIFIED AOC-2 PROCESSOR - EXECUTION STARTED")
    ...
    try:
        ...
        # Use the same extraction function - it works for both AOC-2 and AOC-4
        ...
        return {
            'success': True,
            'message': 'AOC-2 processed and Excel filled successfully',
            'user_id': user_id,
...
def save_extracted_json(bucket, user_id, document_type, document_key, extracted_data):
    ...
    output_data = {
        'metadata': {
            ...
            'processor_version': 'fargate-aoc2-v1.0'
        },
...
def main():
    ...
    """
    log_info("Starting AOC-2 Processor (Fargate Version)")
    ...
    if len(sys.argv) > 1:
        ...
        if len(sys.argv) != 4:
            log_err("Usage: python aoc2_processor.py <bucket> <document_key> <template_key>")
            sys.exit(1)
    ...
    result = process_aoc2(bucket, document_key, template_key)
