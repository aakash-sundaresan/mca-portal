...
def extract_auditor_report_fields_in_chunk_with_bedrock(chunk_text: str, template_data: dict, unprotected_cell: list[str]) -> Dict[str, Any]:
    prompt = f"""
    ...
    **Example (simplified)**:
    ...
    template_data = {{
    "1": {{ 
        "B1": ["1 Report of the auditor", true],
    }}
    "2": {{
            "B2": ["(a) *Opinion of the auditor", true],
    }}
    "3":{{
        "B3": ["", false]
    }}
    "4":{{
        "B4": ["(b) *Basis of opinion"]    
    }}
    "5":{{
        "B5": ["", false]
    }}    
    ...
    For cell B3
    Column scan up B3: Opinion of the auditor -> column header
    Row header No text exists -> no row header
    Column scan of column header: 1 Report of auditor -> second level column header
    Combine them hierarchically:
    "Report of Auditor.Opinion of auditor"
    ...
    Step 2:
    For all the cells in table, using the mapped question, try to extract the complete value if it exists in the document.
    **CRITICAL INSTRUCTIONS TO DERIVE SOME VALUES*
    **Text outputs**
        - Extract the complete text answering the question from the DOCUMENT CHUNK. (ONLY EXTRACT DO NOT INVENT OR MODIFY)
        - If source text is longer than 5000 characters, extract first 4900 characters and concatenate " .Please refer to Auditor's Report for more details" at the end
        - Maximum length: 500 characters
        - Use 'Nil' if text answering the question can't be found in DOCUMENT CHUNK
        - 
    **Numeric outputs**
...
def extract_auditor_report_fields_with_bedrock(document_text: str, template_data: dict) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        while document_start_idx<len(document_text):
            ...
            chunk_extracted_data = extract_auditor_report_fields_in_chunk_with_bedrock(chunk_text, template_data,unprotected_cells)
            for key, value in chunk_extracted_data.items():
                if key in extracted_data:
                    if extracted_data[key] in ("","Nil",0):
...
def extract_fields_with_bedrock(document_text: str, tempate_data: dict) -> dict:
    """
    ...
    try:
        ...
        extracted_fields = extract_auditor_report_fields_with_bedrock(document_text, tempate_data)
