"""
Unified Directors Report Processor - Fargate Version
Processes directors report document and fills Excel template in a single execution

Flow:
1. Extract text from document (Textract)
2. Extract structured fields (Bedrock)
3. Save JSON to S3
4. Load Excel template
5. Map fields to cells intelligently (Bedrock)
...
# Cell mappings
from directors_cell_mappings import prepare_directors_cell_mappings
...
def process_directors_report(bucket, document_key, template_key):
    """
    Main processing function for Directors Report
    """

    log_info("=" * 80)
    log_info("UNIFIED DIRECTORS REPORT PROCESSOR - EXECUTION STARTED")
    ...
    try:
        ...
        if not user_id or not document_type:
            raise ValueError(
                f"Invalid document_key format: {document_key}. Expected: {{userId}}/{{documentType}}/uploads/{{filename}}"
            )
        ...
        return {
            "success": True,
            "message": "Directors report processed and Excel filled successfully",
            "user_id": user_id,
...
def save_extracted_json(bucket, user_id, document_type, document_key, extracted_data):
    ...
    output_data = {
        "metadata": {
            ...
            "processor_version": "fargate-directors-v1.0",
        },
...
def main():
    log_info("Starting Directors Report Processor (Fargate Version)")

    if len(sys.argv) > 1:
        log_info("Reading parameters from command line arguments")
        if len(sys.argv) != 4:
            log_err("Usage: python directors_processor.py <bucket> <document_key> <template_key>")
            sys.exit(1)
    ...
    result = process_directors_report(bucket, document_key, template_key)
    log_info("\n" + "=" * 80)
